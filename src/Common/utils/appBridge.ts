...
import Bridgev2 from './appBridge.v2'
...
export function isApp() {
    if (u.indexOf("besharp") != -1) {
        ...
        return true
    } else if (JSON.parse(sessionStorage.getItem('isBicaiApp') as string) === '1') {
        return true
    } else {
        return false
    }
...
export const Native = {
    ...
    // 去产品详情页
    goProDetail({ data }:Record<string,any>) {
        if (isAndroid) {
            if (window.native) {
    ...
    // 去本行的产品列表
    goBankList({ data }:Record<string,any>, flag:boolean) {
        if (isAndroid) {
            if (window.native) {
    ...
    // 开户成功通知原生
    openInfoSuccess(data:Record<string,any>) {
        if (isAndroid) {
            if (window.native) {
    ...
    // 全行的产品列表(比财app首页)
    goOpenHome({ data }:Record<string,any>) {
        if (isAndroid) {
            if (window.native) {
    ...
    // 活体完成通知app
    BCFaceDiscernResult(isSucceed:string) {
        if (isAndroid) {
            if (window.native) {
                window.native.BCFaceDiscernResult(isSucceed)
                return Promise.resolve('isAndroid')
            } else {
                return Promise.reject("error isAndroid window.native")
            }
            ...
            try {
                if (window.webkit) {
                    window.webkit.messageHandlers.BCFaceDiscernResult.postMessage({"isSucceed":isSucceed})
                    return Promise.resolve('ios')
                } else {
                    return Promise.reject("error isIOS window.webkit")
                }
    ...
    // 比财更新身份证成功
    BCUpdateIDCardSucceed() {
        if (isAndroid) {
            if (window.native) {
                window.native.BCUpdateIDCardSucceed()
                return Promise.resolve('isAndroid')
            } else {
                return Promise.reject("error isAndroid window.native")
            }
            ...
            try {
                if (window.webkit) {
                    window.webkit.messageHandlers.BCUpdateIDCardSucceed.postMessage(" ")
                    return Promise.resolve('ios')
                } else {
                    return Promise.reject("error isIOS window.webkit")
                }
    ...
    // 购买成功页跳转判断
    goBannerUrl(data:Record<string,any>) {
        if (isAndroid) {
    ...
    updateTitle(title:string) {
        ...
        if (isAndroid) {
            if(window.native&&window.native.updateTitle){    //安卓不用h5去设置title他们自己获取
                 window.native.updateTitle( title );
            }else{
    ...
    saveImgAlbumForBack(data:unknown) {
        if (isAndroid) {
