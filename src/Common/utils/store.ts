...
export const StoreKey = {
    changeBankOriginPage:'changeBankOriginPage'
}
...
export const session =  {
    ...
    get(key:string) {
        if (!key) return null
        return JSON.parse(window.sessionStorage.getItem(STORE_PREFIX + key ) as string)
    },
    remove(key:string) {
...
export const local = {
    ...
    set<T>(key:string, value:T, ms = 1000 * 3600 * 24 * 365) {
        if (!key) return false
    ...
    get(key:string) {
        ...
        let value = JSON.parse(window.localStorage.getItem(key) as string)
