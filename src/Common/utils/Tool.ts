...
export class Help {
     ...
     * @returns {*} eg:3,000.00
     */
    formatNum(str:string, flag?:number) {
        if (!str) {
            if (flag == 1) {
                return '--'
            } else {
    ...
    */
     ...
     * @returns 相减结果
     */
    accSub(arg1:number, arg2:number):any {
        var r1, r2, m, n;
    ...
    StorageAddressBar = () => {
        session.set('changeBankOriginPage', window.location.href.split('#/')[1])
    }
}
