...
export class updateIdCard extends StoreExtends {
    ...
    @observable nationVal: any[] = ['']            //民族
    @observable gender = ['']               //性别
    @observable signOrg = ""                //签发机关
    @observable address: string = ""                //地址
    ...
    imgToBase = (e: { target: { files: any[]; }; }, type: string) => {
    ...
    //修改身份证号
    changeIdCard = (val: string) => {
        runInAction(() => {
            this.userCardId = val
        })
    }
    //修改签发机关
    changeIssuingAuthority = (val: string) => {
        runInAction(() => {
            this.signOrg = val
        })
    }
    ...
    //修改民族
    changeNation = (val: any[]) => {
        runInAction(() => {
            this.nationVal = val
        })
    }
    //修改出生日期
    changeBirthday = (val: any) => {
        runInAction(() => {
            this.birthday = val
        })
    }

    changeValueStart = (val: any) => {
        runInAction(() => {
            this.pickerValueStart = val
        })
    }

    changeValueEnd = (val: any) => {
        runInAction(() => {
            this.pickerValueEnd = val
        })
    }
    //民族遍历出来数据
    getData = (val: { nationList: any[]; }) => {
        runInAction(() => {
    ...
    uploadCardImg = async (params: any) => {
    ...
    apiUpdateIdCardFn = async (openErr?: INNER_CODE | undefined, yzm?: any) => {
     ...
     * 更新身份证需要的参数
     */
    apiUpdateIdCardParams(openErr?: string | undefined, yzm?: undefined) {
        // if(!openErr) return
    ...
    catchErr = (openErr: INNER_CODE | undefined, err: any) => {
        // if(false){
    ...
    gobackPage = (openErr: INNER_CODE.SubmitOnly | INNER_CODE.ModifyInfo | INNER_CODE.ModifyOpenInfo | INNER_CODE.SubmitAndDoThing | INNER_CODE.CancelAndCallPhone | INNER_CODE.CancelAndDoWithdraw | INNER_CODE.CancelAndDoRedeem | INNER_CODE.CancelAndReAssess | INNER_CODE.DoFaceDiscern | INNER_CODE.Signing | INNER_CODE.goSuidePage | INNER_CODE.goBcFaceDiscern | INNER_CODE.LargeAmounts | INNER_CODE.CancelAndCallBcPhone | INNER_CODE.PasswordNotSet | undefined | INNER_CODE.CancelAndUpdateIdCard) => {
